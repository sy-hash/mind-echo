...
erDiagram
    ...
    Recording {
        ...
        Optional_String transcription "書き起こしテキスト（nil = 未実行）"
    }
...
enum ShareType {
    ...
    case transcription      // 書き起こしテキスト（全録音の transcription を結合した .txt）
}
...
- **今日の録音リスト（下部）** — 今日既に録音がある場合、各録音を連番で一覧表示:
  - 連番（#1, #2, ...）
  - 録音時間
  - 書き起こしテキストのプレビュー（保存済みの場合。1行に収まるよう末尾を省略表示）
  - セルタップで再生 / 停止（個別再生）
  - 書き起こしボタン（保存済みテキストがない場合のみ表示）
...
7. 書き起こしを非同期で実行し、完了後 `Recording.transcription` に保存
8. 再度録音開始ボタンを押すと → 新しいファイル・新しい `Recording` として別セッションで録音開始
...
- 録音セクション — 各 Recording を連番順にリスト表示:
  ...
  - セルタップで再生 / 停止（個別再生）
  - 録音時間の表示
  - 書き起こしテキスト（`Recording.transcription` が non-nil の場合は保存済みテキストを表示。nil の場合は「書き起こし」ボタンを表示し、タップで書き起こしを実行・保存）
...
録音の書き起こし（Speech → テキスト変換）結果を `Recording.transcription` に永続化し、以降は保存済みテキストを参照する。

### 保存タイミング

録音停止後に書き起こしを実行し、完了時に `Recording.transcription` へ保存する。

```
録音停止
  ↓
Recording エンティティを作成（transcription = nil）
  ↓
書き起こしを非同期で実行（Speech フレームワーク）
  ↓
完了 → Recording.transcription にテキストを保存
  ↓
UI に書き起こし結果を表示
```
...
書き起こしテキストの表示時は、以下の優先順位で判定する:

1. `Recording.transcription` が non-nil → 保存済みテキストをそのまま表示（書き起こしを再実行しない）
2. `Recording.transcription` が nil → 未書き起こし状態。ユーザー操作で書き起こしを実行可能

### パーミッション

| キー | 用途 |
|------|------|
| `NSSpeechRecognitionUsageDescription` | 音声の書き起こしのために音声認識を使用します |

### スキーマ移行

`Recording.transcription` は `String?`（Optional）のため、SwiftData の lightweight migration で自動的に追加される（既存レコードは `nil`）。`VersionedSchema` による明示的なマイグレーション定義は不要。
...
| 書き起こしテキスト | `.txt` | 全 Recording の `transcription` を `sequenceNumber` 順に結合して生成 | テキストソースとして読み込み |

※ NotebookLM は m4a, mp3, wav, aac 等の音声ファイルを直接ソースとしてインポート可能。
音声をアップロードすると NotebookLM 側でも自動で文字起こしが行われる。
※ 書き起こしテキスト共有は、全 Recording に `transcription` が保存済みの場合のみ有効。未書き起こしの Recording がある場合はユーザーに通知する。
...
共有ボタンタップ
  ↓
共有タイプを選択（音声 / 書き起こしテキスト）
  ↓
エクスポートデータを生成:
  - 音声: 日付アナウンスを TTS で生成し、冒頭に挿入 + 全 Recording を連番順に結合 → 1つの .m4a
  - テキスト: 全 Recording の transcription を sequenceNumber 順に結合 → 1つの .txt
  ↓
...
- 書き起こしテキスト:
  1. 全 Recording の `transcription` を `sequenceNumber` 順に結合
  2. 各録音を `#N` の区切りで連結し、冒頭に日付ヘッダーを付与
  3. `Documents/Exports/` に `.txt` として保存
...
**書き起こしテキスト** — ファイル名: `20250207_transcription.txt`

```
2025-02-07 Fri

#1
今日はミーティングがあって、新しいプロジェクトの話をした。

#2
夕方にジムに行った。最近運動不足だったので良いリフレッシュになった。
```
