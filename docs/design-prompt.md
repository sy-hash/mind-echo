...
- **リアルタイムスレッド安全性**: tap コールバックはリアルタイムオーディオスレッドで実行されるため、コールバック内ではメモリ確保・ロック取得・Objective-C メッセージングなどのブロッキング操作を最小限に抑える。バッファコピー後の外部コールバック呼び出しは `DispatchQueue` 経由で非リアルタイムスレッドにディスパッチすることを推奨する
...
inputNode.installTap(onBus: 0, bufferSize: 1024, format: recordingFormat) { buffer, time in
    ...
    // 非リアルタイムスレッドにディスパッチしてから外部コールバックを呼ぶ
...
- **リアルタイムスレッドからのディスパッチ**: tap コールバック内で直接 `onBuffer` クロージャを呼ぶと、呼び出し先でメモリ確保（`malloc`）、Objective-C のメッセージ送信、ロック取得などのブロッキング操作が発生し、オーディオグリッチ（音飛び・ノイズ）の原因になる。バッファコピー後は `DispatchQueue.main.async`（または専用のシリアルキュー）で非リアルタイムスレッドにディスパッチしてから外部コールバックを呼ぶこと。`AVAudioFile.write(from:)` はリアルタイムスレッドでの呼び出しが許容されている API である
